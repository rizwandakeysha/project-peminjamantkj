  ...
  try {
    ...
    const result = await db.query(
      `INSERT INTO jenis_barang (kode_jenis_barang, nama_jenis_barang, deskripsi_jenis_barang) 
       VALUES ($1, $2, $3) RETURNING id_jenis_barang`,
...
exports.deleteJenisBarang = async (req, res) => {
  try {
    ...
    // Check if jenis has related barang
    const barangResult = await db.query(
      'SELECT * FROM barang WHERE id_jenis_barang = $1',
      [id]
    );
    ...
    if (barangResult.rows.length > 0) {
      return res.status(400).json({
        success: false,
        message: 'Tidak dapat menghapus jenis barang yang memiliki barang terkait',
      });
    }

    const result = await db.query(
      'DELETE FROM jenis_barang WHERE id_jenis_barang = $1',
      [id]
    );
