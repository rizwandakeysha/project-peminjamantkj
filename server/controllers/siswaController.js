...
exports.getSiswaByKelas = async(req, res) => {
    try {
        ...
        const result = await db.query(
            'SELECT id_siswa as id, nis, nama_siswa as name, kelas, created_at FROM siswa WHERE kelas = $1 ORDER BY nama_siswa ASC', [kelas]
        );
...
exports.getSiswaById = async(req, res) => {
    try {
        const { id } = req.params;
        const result = await db.query(
            'SELECT id_siswa as id, nis, nama_siswa as name, kelas, created_at FROM siswa WHERE id_siswa = $1', [id]
        );
...
exports.getSiswaByNis = async(req, res) => {
    try {
        ...
        const result = await db.query(
            'SELECT id_siswa as id, nis, nama_siswa as name, kelas, created_at FROM siswa WHERE nis = $1', [nis]
        );
...
exports.createSiswa = async(req, res) => {
    try {
        ...
        const result = await db.query(
            'INSERT INTO siswa (nis, nama_siswa, kelas) VALUES ($1, $2, $3) RETURNING id_siswa', [nis, name, kelas || null]
        );
        ...
        const newSiswaResult = await db.query(
            'SELECT id_siswa as id, nis, nama_siswa as name, kelas, created_at FROM siswa WHERE id_siswa = $1', [newId]
        );
...
exports.updateSiswa = async(req, res) => {
    try {
        const { id } = req.params;
        const { name, nis, kelas } = req.body;

        const result = await db.query(
            'UPDATE siswa SET nama_siswa = $1, nis = $2, kelas = $3 WHERE id_siswa = $4', [name, nis, kelas || null, id]
        );
