...
exports.getAllGuru = async(req, res) => {
    try {
        const result = await db.query(
            'SELECT id, nip, nama_guru as name, created_at FROM guru ORDER BY nama_guru ASC'
        );
        res.json({
            success: true,
            data: result.rows,
...
exports.getGuruById = async(req, res) => {
    try {
        const { id } = req.params;
        const result = await db.query(
            'SELECT id, nip, nama_guru as name, created_at FROM guru WHERE id = $1', [id]
        );
...
exports.getGuruByNip = async(req, res) => {
    try {
        ...
        const result = await db.query(
            'SELECT id, nip, nama_guru as name, created_at FROM guru WHERE nip = $1', [nip]
        );
...
exports.createGuru = async(req, res) => {
    try {
        ...
        const result = await db.query(
            'INSERT INTO guru (nip, nama_guru) VALUES ($1, $2) RETURNING id', [nip, name]
        );
        ...
        const newGuruResult = await db.query(
            'SELECT id, nip, nama_guru as name, created_at FROM guru WHERE id = $1', [newId]
        );
...
exports.updateGuru = async(req, res) => {
    try {
        ...
        const result = await db.query(
            'UPDATE guru SET nama_guru = $1, nip = $2 WHERE id = $3', [name, nip, id]
        );
...
exports.deleteGuru = async(req, res) => {
    try {
        ...
        const result = await db.query(
            'DELETE FROM guru WHERE id = $1', [id]
        );
